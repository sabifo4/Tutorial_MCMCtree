...
# approaches.
num_dirs    <- 1
path_prior  <- paste( home_dir, "sum_analyses/prior/mcmc_files_CLK", sep = "" )

# Load semicolon-separated file/swith info about calibrated nodes. Note that
...
# of the file/s that you have used. Argument `dat` requires the same global object
# that you have created at the beginning of the script. If your input files 
# have a header, please keep `head_avail = TRUE`. Otherwise, change this to 
# FALSE.
...
for( i in 1:num_dirs ){
  count <- count+1
  cat( "\n[[ Generating plots for dataset ", names( mcmc_priors )[count], " ]]\n" )
  plot_check_calibnodes( calibs = calib_nodes[[ 1 ]], divt_list = mcmc_priors[[ i ]], 
