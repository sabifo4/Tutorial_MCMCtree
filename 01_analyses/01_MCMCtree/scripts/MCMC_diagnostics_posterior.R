...
## POSTERIOR ##
path_post_GBM  <- paste( home_dir, "sum_analyses/posterior/1/GBM/", sep = "" )
path_post_ILN  <- paste( home_dir, "sum_analyses/posterior/1/ILN/", sep = "" )
...
post_GBM_half2 <- apply( X = post_GBM_sum$mean[(half_chains+1):num_chains,],
                         MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir, "ESS_and_chains_convergence/Convergence_plot_posterior_GBM.pdf", sep = "" ),
     paper = "a4" )
...
min( ESS_GBM$stats$Rhat ) # 0.9999712
max( ESS_GBM$stats$Rhat ) # 1.000077
...
post_ILN_half2 <- apply( X = post_ILN_sum$mean[(half_chains+1):num_chains,],
                         MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir, "ESS_and_chains_convergence/Convergence_plot_posterior_ILN.pdf", sep = "" ),
     paper = "a4" )
...
## Max.    50001    50232
# Calculate Rhat, min and max. Good if max Rhat <= 1.05
min( ESS_ILN$stats$Rhat ) # 0.9999999
max( ESS_ILN$stats$Rhat ) # 1.000027
