...
# X will be an integer (i.e., 5). Subtract two to this number (i.e., 5-2=3) and
# this will be your number of divergence times that are parameters of the MCMC.
...
## PRIOR ##
path_prior  <- paste( home_dir, "sum_analyses_prot/prior/1/CLK/", sep = "" )
...
prior_sum <- sum_MCMC( num_dirs = num_chains, delcol = delcol_prior,
                       data_dir = path_prior,
...
prior_half1 <- apply( X = prior_sum$mean[1:half_chains,],
                      MARGIN = 2, FUN = mean )
prior_half2 <- apply( X = prior_sum$mean[(half_chains+1):num_chains,],
                      MARGIN = 2, FUN = mean )
pdf( paste( outchecks_dir, "ESS_and_chains_convergence/Convergence_plot_prior_prot.pdf", sep = "" ),
     paper = "a4" )
plot_convergence( name_dir_dat = "CLK, prior",
...
th <- 0.1
sum_quantiles <- check_quantiles( dat = prior_sum, threshold = th,
...
check_quantiles( dat = prior_sum, threshold = th, main_chain = main_ch,
...
#              file = paste( path_prior, "chains_kept.txt",
#                            sep = "" ),
#              quote = FALSE, row.names = FALSE, col.names = FALSE )
# prior_FILT_sum  <- sum_MCMC( num_dirs = filt_chains, delcol = delcol_prior,
#                              data_dir = path_prior,
#                              num_divt = num_divt, node_calib = calib_nodes,
#                              dataset = "CLK_filt",
#                              perc = perc, def_samples = def_samples,
...
# pdf( paste( outchecks_dir, "ESS_and_chains_convergence/Convergence_plot_prior_prot_filt.pdf", sep = "" ),
#      paper = "a4" )
# plot_convergence( name_dir_dat = "Prior filtered chains - CLK",
#                   mean_divt1 = prior_FILT_sum$mean[1,],
#                   mean_divt2 = prior_FILT_sum$mean[2,],
#                   num_runs = length(filt_chains) )
# dev.off()
# #> Delete old prior object with unfiltered chains and keep the new one as main
# rm( prior_sum )
# prior_sum <- prior_FILT_sum
...
ESS_CLK <- sum_MCMC_ESS( x = prior_sum$arr4stan,
                             samp_per_chain = prior_sum$samp_per_chain )
# Minimum samples per chain
min( prior_sum$samp_per_chain  )    # 20001
# Median samples per chain
median( prior_sum$samp_per_chain  ) # 20001
# Maximum samples per chain
max( prior_sum$samp_per_chain  )    # 20001
# Number of samples used to compute the ESS
min( prior_sum$samp_per_chain  ) * num_chains # 100005
...
min( ESS_CLK$stats$Rhat ) # 0.9999842
max( ESS_CLK$stats$Rhat ) # 1.000036
# Number of samples collected throughout
dim( prior_sum$all_mcmc ) # 100005      3
