...
# Path to tree
path_tree <- c( "../../00_inp_data/mtCDNApri.trees" )
...
for( c in 1:length( calibrations ) ){
  ...
  writeLines( text = paste( length(tt_all[[c]]$tip.label ), " 1", sep = "" ), 
              con = paste( "../../00_inp_data/calibs/raw_calibs/cals_only_",
              names( calibrations )[c], ".tree", sep = "" ) )
  ape::write.tree( phy = tt_all[[c]],
                   file = paste( "../../00_inp_data/calibs/raw_calibs/cals_only_",
                   names( calibrations )[c], ".tree", sep = "" ),
                   append = TRUE )
}
...
tt_name <- c( "../../00_inp_data/calibs/raw_calibs/cals_only_mtcdnapri.tree" )
for( t in 1:length( tt_name ) ){
  ...
  #-------------------------------#
  out_dir <- "../../00_inp_data/tree_display/"
  if( ! dir.exists( "../../00_inp_data/tree_display/" ) ){
    dir.create( "../../00_inp_data/tree_display/" )
  }
