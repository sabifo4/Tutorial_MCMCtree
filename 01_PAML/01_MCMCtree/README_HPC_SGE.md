...
Now, we will transfer our in-house bash scripts to run `MCMCtree` and corresponding template files with which we will parse our data (available under [`scripts`](scripts)) to the `example_dating` directory in the HPC server:

```sh
# Run from `01_PAML/01_MCMCtree/scripts` in your local PC so that you can
# transfer these scripts to the HPC server you are uing
...
```sh
# Run from the `example_dating` dir (HPC)
# Please change directories until
# you are there. Then run the following
# commands.
home_dir=$( pwd )
...
for i in `seq 1 $num_aln`
do
./generate_job_MCMCtree_HPC_SGE.sh $i GBM 1 $home_dir/pipelines_MCMCtree mcmctree $num_chains example_dating 0 Y
./generate_job_MCMCtree_HPC_SGE.sh $i ILN 1 $home_dir/pipelines_MCMCtree mcmctree $num_chains example_dating 0 Y
done
...
```sh
# Run from `pipelines_MCMCtree_prior` (HPC)
# Please change directories until
# you are there. Then run the following
# commands.
# 
...
```sh
# Run from `pipelines_MCMCtree_prior/1/CLK` (HPC)
# Please change directories until
# you are there. Then run the following
# commands.
chmod 775 *sh
...
```sh
# Run from `01_MCMCtree/scripts` (local PC)
chmod 775 *sh
...
```sh
# Run from `sum_analyses/00_prior` (local PC)
name_dat=( 'example' )
num_dat=1
count=-1
for i in `seq 1 $num_dat`
...
```sh
# Run from `01_MCMCtree/calib_files` (local PC)
...
# and run the following command (HPC)
# Please change directories until
# you are there.
chmod 775 *sh
...
```sh
# Run from `example_dating` directory (HPC)
...
Now, you can transfer the temporary directory to your `01_PAML/01_MCMCtre` directory on your local PC, e.g., using `rsync`:
...
```sh
# Run from `sum_analyses_prot/01_posterior` directory (local PC)
# Please change directories until
# you are there. Then run the following
# commands.
name_dat=( 'example' )
