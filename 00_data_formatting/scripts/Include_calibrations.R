...
names( calibrations ) <- names( keep_indexes )  <- names( ind_dup ) <- 
  names( nodes_dup ) <- names( tt_all ) <- c( "example" )

# Path to tree
path_tree <- c( "../01_inp_data/tree_example_uncalib.tree" )
...
tt_name <- c( "../00_raw_data/trees/cals_only_example.tree" )
for( t in 1:length( tt_name ) ){
  ...
  write( x = phylip.header, file = paste( "../00_raw_data/trees/",
                                          ...
                                          sep = "" ) )
  write( x = tt2, file = paste( "../00_raw_data/trees/", out_fullname,
